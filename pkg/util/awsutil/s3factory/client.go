...
func NewClientFromSecret(kubecli kubernetes.Interface, namespace, awsEndpoint, awsSecret string) (w *S3Client, err error) {
	defer func() {
		if err != nil {
	...
	so, err := setupAWSConfig(kubecli, namespace, awsSecret, awsEndpoint, w.configDir)
	if err != nil {
