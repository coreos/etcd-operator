...
func (c *Cluster) reconcile(pods []*api.Pod, op ServiceType) error {
	c.logger.Infoln("Start reconciling")
	defer c.logger.Infoln("Finish reconciling")

	switch {
	case len(pods) != c.Spec.Size:
		running := member.GetMemberSet(op)
		for _, pod := range pods {
			m := &member.Member{Name: pod.Name}
...
func (c *Cluster) reconcileSize(running member.MemberSet) error {
...
func (c *Cluster) addOneMember() error {
	...
	newMember := &member.Member{Name: newMemberName}
...
func (c *Cluster) removeMember(toRemove *member.Member) error {
...
func (c *Cluster) disasterRecovery(left member.MemberSet) error {
	if c.Spec.SelfHosted != nil {
...
func pickOneOldMember(pods []*api.Pod, newVersion string) *member.Member {
	for _, pod := range pods {
		if k8sutil.GetEtcdVersion(pod) == newVersion {
			continue
		}
		return &member.Member{Name: pod.Name}
	}
	return nil
}
