...
Create a secret using the `peer.key` and `peer-ca.crt`:
...
Create a secret using `server.key` and `server-ca.crt`:
...
The operator must send the client requests (such as snapshot, healthy check, add/remove member) to maintain this cluster.
...
Both `etcd-client.crt` and `etcd-client.key` should be trusted by the etcd server's client CA `server-ca.crt`.

`etcd-client-ca.crt` should trust the etcd server's key and cert.
...
[example-tls]: https://github.com/coreos/etcd-operator/tree/master/example/tls/
